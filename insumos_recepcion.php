<?php
// Archivo: insumos_recepcion.php
// Propósito: Recepción Insumos con ALTA RÁPIDA EN TABLA

require 'db.php';
session_start();

if (!isset($_GET['id']) || empty($_GET['id'])) { header("Location: insumos_compras.php"); exit; }
$id_oc = $_GET['id'];
$roles_usuario = $_SESSION['user_roles'] ?? [];

// Permisos
if (!in_array('Administrador', $roles_usuario) && !in_array('Encargado Depósito Insumos', $roles_usuario)) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['accion']) && $_POST['accion'] == 'crear_rapido') {
    try {
        $stmtNew = $pdo->prepare("INSERT INTO insumos_medicos (codigo, nombre, descripcion, unidad_medida, stock_actual, stock_minimo) VALUES (:c, :n, 'Alta rápida recepción', 'unidades', 0, 10)");
        $stmtNew->execute([':c' => $_POST['nuevo_codigo'], ':n' => $_POST['nuevo_nombre']]);
        $mensaje = '<div class="alert alert-success alert-dismissible fade show">✅ <strong>' . htmlspecialchars($_POST['nuevo_nombre']) . '</strong> agregado. Ya puedes seleccionarlo.<button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>';
    } catch (Exception $e) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['accion']) && $_POST['accion'] == 'recibir') {
    try {
        ...
        foreach ($items_recibidos as $id_item_oc => $datos) {
            ...
            if ($id_insumo > 0) {
                // Stock
                $stmtStock = $pdo->prepare("UPDATE insumos_medicos SET stock_actual = stock_actual + :cant WHERE id = :id");
                $stmtStock->execute([':cant' => $cant_recibida, ':id' => $id_insumo]);

                // Item OC
                $stmtItem = $pdo->prepare("UPDATE ordenes_compra_items SET cantidad_recibida = :cant, id_insumo_asociado = :id_ins WHERE id = :id");
                $stmtItem->execute([':cant' => $cant_recibida, ':id_ins' => $id_insumo, ':id' => $id_item_oc]);

                // Faltante
                if ($cant_recibida < $cant_pedida) {
                    $hubo_faltantes = true;
                    $diff = $cant_pedida - $cant_recibida;
                    $stmtNom = $pdo->prepare("SELECT nombre FROM insumos_medicos WHERE id = :id");
                    $stmtNom->execute(['id' => $id_insumo]);
        ...
        // Notificaciones (Compras + Director Médico)
        if ($hubo_faltantes) {
            ...
            $stmtNoti->execute([':uid' => $datosOC['id_usuario_creador'], ':msj' => $msj, ':url' => "insumos_oc_ver.php?id=" . $id_oc]);
            ...
            if ($rolDir) {
                ...
                $stmtNoti2->execute([':rol' => $rolDir, ':msj' => $msj, ':url' => "insumos_oc_ver.php?id=" . $id_oc]);
            }
        }

        $pdo->commit();
        header("Location: insumos_stock.php?msg=recepcion_ok");
        exit;
...
$stmt = $pdo->prepare("SELECT * FROM ordenes_compra WHERE id = :id AND tipo_origen = 'insumos'");
$stmt->execute(['id' => $id_oc]);
$orden = $stmt->fetch();

if ($orden['estado'] != 'aprobada_logistica' && $orden['estado'] != 'recibida_parcial') {
    echo "<div class='container mt-5 alert alert-warning'>Orden no lista.</div>"; include 'includes/footer.php'; exit;
}
...
$lista_insumos = $pdo->query("SELECT id, nombre, codigo FROM insumos_medicos ORDER BY nombre ASC")->fetchAll();
?>

<div class="container-fluid px-4">
    <h1 class="mt-4">Recepción Insumos Médicos</h1>
    <?php echo $mensaje; ?>

    <form method="POST" action="">
        <input type="hidden" name="accion" value="recibir">
        <div class="card mb-4 border-primary">
            <div class="card-header bg-primary text-white fw-bold"><i class="fas fa-truck-loading"></i> Ingreso de Mercadería</div>
            <div class="card-body">
                <div class="row mb-3">
                    <div class="col-md-4">
                        <label class="fw-bold">N° Remito *</label>
                        <input type="text" name="numero_remito" class="form-control" required placeholder="Ej: REM-MED-001">
                    </div>
                    <div class="col-md-8 d-flex align-items-center">
                        <div class="alert alert-info w-100 mb-0 py-2 small">
                            <i class="fas fa-info-circle"></i> Si el insumo es nuevo, usa el botón <strong>"+ Nuevo"</strong> en la columna "Vincular".
                        </div>
                    </div>
                </div>
                    ...
                    <thead class="table-light">
                        <tr>
                            ...
                            <th style="min-width: 300px;">
                                ...
                                <button type="button" class="btn btn-sm btn-primary float-end" data-bs-toggle="modal" data-bs-target="#modalAltaRapida">
                    ...
                    <tbody>
                        <?php foreach ($items_orden as $item): ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <select name="recibido[<?php echo $item['id']; ?>][id_insumo]" class="form-select select-search" required>
                                        <option value="">-- Seleccionar --</option>
                                        <?php foreach ($lista_insumos as $s): ?>
                                ...
                                <td>
                                    ...
                                    <input type="number" name="recibido[<?php echo $item['id']; ?>][cantidad]" class="form-control fw-bold text-primary" value="<?php echo $item['cantidad_solicitada']; ?>" min="0" onchange="verificarFaltante(this, <?php echo $item['cantidad_solicitada']; ?>)">
            ...
            <div class="card-footer text-end"><button type="submit" class="btn btn-primary btn-lg">Confirmar Recepción</button></div>
        </div>
    </form>
</div>
...
<div class="modal fade" id="modalAltaRapida" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-primary text-white"><h5 class="modal-title">Crear Nuevo Insumo</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
            <form method="POST">
                ...
                <div class="modal-footer"><button type="submit" class="btn btn-primary">Guardar y Usar</button></div>
            </form>
        </div>
    </div>
</div>
